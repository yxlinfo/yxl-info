document.addEventListener("DOMContentLoaded", () => {
  console.log("YXLInfo app.js loaded (v34)");
  /* =========================
     Config
  ========================= */
...
const on = localStorage.getItem(KEY_ON) === "1";
  ...
  function initHallOfFame() {
    ...
    const HOF = [
      ...
      { gen: "19등", name: "JS2" },
      { gen: "20등", name: "낭로우로우로" },
      { gen: "1대부장", name: "류레카시아∮", cnt: "4,698,914개" },
      { gen: "2대부장", name: "류레카시아∮", cnt: "3,070,017개" },
      { gen: "3대부장", name: "류레카시아∮", cnt: "3,687,480개" },
      ...
      { gen: "10대부장", name: "지유님♥", cnt: "4,001,954개" }
    ];
  ...
  const YXL_SCHEDULE = [
    ...
    { date: "2025-12-27", time: "17:00", type: "이벤트", title: "SOOP 스트리머 대상" },
  ...
  // KST 기준으로 날짜(00:00)를 잡아 월/주 경계가 어긋나지 않게
  function kstDate00() {
  ...
  function addMonths(d, n) {
    // 항상 1일 앵커로 고정(월 말 일수 차이로 인한 튐 방지)
  ...
  function initYxlSchedule() {
    ...
    // 달력(월간 그리드)에는 기본적으로 모든 일정을 표시합니다.
    // (차후 "생일만", "엑셀만" 같은 필터가 생기면 여기서 제한하면 됩니다.)
    ...
    function renderMonth() {
      rangeEl.textContent = fmtMonth(monthAnchor);
      grid.innerHTML = "";
...
const isHoliday = isKoreanHoliday(ymd);
        ...
        col.innerHTML = `
          <div class="schHead">
            <div class="schHeadLeft">
              ...
              <span class="schDow">${DOW[dowIdx]}</span>
            </div>
            <div class="schRight">
        ...
        col.addEventListener("click", () => {
          // 다른 달 날짜를 클릭하면 해당 달로 자연스럽게 이동
          if (isOut) monthAnchor = new Date(d.getFullYear(), d.getMonth(), 1);
          activeYMD = ymd;
          renderMonth();
    ...
    btnToday?.addEventListener("click", () => {
      const t = kstDate00();
      monthAnchor = new Date(t.getFullYear(), t.getMonth(), 1);
      activeYMD = toYMD(t);
      renderMonth();
      renderDetail(activeYMD);
    });
