...
def init_db_if_empty():
    ...
    if cur.fetchone()[0] == 0:
        ...
        members = [
            ...
            ('서니', 'iluvpp', 11, 'https://storage2.ygosu.com/?code=S69f0a954b0c8f6.90064035', '1997년생', '2025.06.17 (재입사 2026.04.14)', '160cm', 'ISTP', ''),
...
def get_members_from_db():
    # 💡 이미 존재하는 DB라도 서니님 프로필 데이터 강제 반영을 위한 업데이트 쿼리
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("UPDATE Members SET join_date = '2025.06.17 (재입사 2026.04.14)' WHERE name = '서니'")
    conn.commit()
    ...
    for r in rows:
        ...
        # 💡 서니님을 위한 커스텀 D-DAY 계산 로직 (기존 271일 + 최근 입사일부터 경과일)
        if name == '서니':
            try:
...
def generate_full_system(members, history_db):
    ...
    # 💡 VOD 캐싱 로직: 아이디 배열이 같으면 API를 건너뛰고 기존 저장된 파일을 읽음
    ...
    if os.path.exists(cache_file):
        try:
            ...
            if cached_data.get("ids") == vod_ids:
                ...
                print("⚡ 저장된 VOD 캐시를 성공적으로 불러왔습니다. (API 호출 스킵)")
                need_fetch = False
        except Exception as e:
            print("⚠️ 캐시 읽기 실패, 새로 가져옵니다.", e)
            
    if need_fetch:
        print("⏳ 새로운 VOD 데이터를 API로 가져오는 중 (약간의 시간이 소요됩니다)...")
...
<head>
    ...
    <style>
        ...
        .stat-value {{ font-size: 14px; color: #fff; text-align: right; word-break: keep-all; }}
