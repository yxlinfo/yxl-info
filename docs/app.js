...
if (q) {
  ...
  function renderSeason() {
    ...
    const findHeaderByPrefix = (prefix) => {
      const p = normalize(prefix);
      return headers.find((h) => {
        const nh = normalize(h);
        return nh === p || nh.startsWith(p);
      }) || prefix;
    };

    // ✅ 괄호/날짜가 붙은 헤더도 인식 (예: '1회차(11.11)')
    const _srcBeforeKey = findHeaderByPrefix("직급전");
    const _srcRounds = [1, 2, 3, 4, 5].map((n) => findHeaderByPrefix(`${n}회차`));
    const _srcSumKey =
      findHeaderByPrefix("합산기여도") ||
      findHeaderByPrefix("누적기여도") ||
      "합산기여도";
  ...
  async function loadMainExcel() {
    ...
    // ✅ 괄호/날짜가 붙은 헤더도 인식 (예: '1회차(11.11)')
    const iHeaders = tIntegrated.headers || [];
    const findIKey = (prefix) => {
      const p = normalize(prefix);
      return iHeaders.find((h) => {
        const nh = normalize(h);
        return nh === p || nh.startsWith(p);
      }) || prefix;
    };
    ...
    state.main.integratedAll = (tIntegrated.rows || []).map((r) => {
      ...
      let sum = pick(r, [findIKey("합산기여도"), "합산기여도", "합산 기여도"]);
      if (sum == null || sum === "") {
        const parts = ["직급전", "1회차", "2회차", "3회차", "4회차", "5회차"].map((k) => Number(pick(r, [findIKey(k), k]) ?? 0) || 0);
